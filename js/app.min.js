...
window.addEventListener('DOMContentLoaded',function(){const e=document.getElementById('header-navbar');window.location.pathname.endsWith('form.html')?e.classList.add('dark'):window.addEventListener('scroll',function(){let t=null;for(const n of document.querySelectorAll('section')){const o=n.getBoundingClientRect();if(o.top<=e.offsetHeight&&o.bottom>=0){t=n;break}}t&&'rgb(255, 255, 255)'===window.getComputedStyle(t).backgroundColor?e.classList.add('dark'):e.classList.remove('dark')})}),$(document).ready(function(){let e=0,t=$('.form-step'),n=$('.progress-indicator .step'),o=[];t.hide(),$(t[e]).show(),$(n[e]).addClass('active-step');const a=e=>{let t=!0;return e.each(function(){$(this).val()||(t=!1)}),t},s=()=>{$(t[e]).hide(),$(n[e]).removeClass('active-step'),e++,e<t.length&&($(t[e]).show(),$(n[e]).addClass('active-step'))},c=()=>{$(t[e]).hide(),$(n[e]).removeClass('active-step'),e--,e>=0&&($(t[e]).show(),$(n[e]).addClass('active-step'))};$('.back-btn').click(c);const r=r=>{if('submit'===r.target.type)return;const c=$(t[e]).find('input, select, textarea'),l=a(c);if(l){const e={};c.each(function(){e[$(this).attr('name')]=$(this).val()}),o[e]=e,s()}else{$('#multi-step-form').append('<div class="alert">Please fill out all required fields.</div>'),setTimeout(()=>$('.alert').remove(),1e3)}e<t.length-1&&r.preventDefault()};$('.next-btn, .submit').click(r);var l=document.getElementById("multi-step-form");async function i(a){a.preventDefault();var s=document.getElementById("my-form-status"),c=new FormData(a.target);fetch(a.target.action,{method:l.method,body:c,headers:{Accept:'application/json'}}).then(function(a){a.ok?(s.innerHTML="Thanks for your submission!",l.reset()):a.json().then(function(a){Object.hasOwn(a,'errors')?s.innerHTML=a.errors.map(function(e){return e.message}).join(", "):s.innerHTML="Oops! There was a problem submitting your form"})}).catch(function(){s.innerHTML="Oops! There was a problem submitting your form"})}l.addEventListener("submit",i)});
...
function submitForm() {
    ...
    fetch('https://getform.io/f/pboxrdra', {
        method: 'POST',
        body: formData
...
document.querySelector('.btn.btn-secondary.btn-lg').addEventListener('click', function(event) {
