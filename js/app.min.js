    ...
    if (window.location.pathname.endsWith('form.html')) {
        ...
        window.addEventListener('scroll', function() {
            ...
            for (const section of sections) {
                ...
                if (position.top <= navbar.offsetHeight && position.bottom >= 0) {
                    currentSection = section;
                    break;
                }
            }

            if (currentSection && window.getComputedStyle(currentSection).backgroundColor === 'rgb(255, 255, 255)') {
                navbar.classList.add('dark');
            } else {
...
$(document).ready(function() {
    ...
    const progressSteps = $('.progress-indicator .step');
    const responses = [];
    steps.hide();
    $(steps[currentStep]).show();
    $(progressSteps[currentStep]).addClass('active-step');
    ...
    const advanceStep = () => {
        ...
        $(progressSteps[currentStep]).removeClass('active-step');
        currentStep++;
    ...
    $('.back-btn').click(goBackStep);

    const handleStep = (e) => {
        e.preventDefault();

        const currentStepInputs = $(steps[currentStep]).find('input, select, textarea');
        const isValid = validateInputs(currentStepInputs);
        ...
        if (isValid) {
            ...
            $('#multi-step-form').append('<div class="alert">Please fill out all required fields.</div>');
            setTimeout(() => $('.alert').remove(), 1000);
        }
    }

    $('.next-btn').click(handleStep);
